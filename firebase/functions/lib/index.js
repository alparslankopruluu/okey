...
export const reserveUsername = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (currentNormalized === normalized && target.data()?.uid === userId)
            return;
        if (target.exists)
            throw new HttpsError('already-exists', 'Username unavailable');
        try {
            assertUsernameChangeAllowed(profile.data()?.usernameChangedAt, now);
        }
        catch {
            throw new HttpsError('failed-precondition', 'Username can change once every 30 days');
        }
        if (currentNormalized.length > 0)
            transaction.delete(db.doc(`usernames/${currentNormalized}`));
...
export const searchUsers = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    const hidden = new Set(blocked.docs.flatMap((doc) => doc.data().members ?? []));
...
export const sendFriendRequest = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (block.exists)
            throw new HttpsError('permission-denied', 'Blocked users cannot connect');
        if (pair.exists)
            throw new HttpsError('already-exists', 'Friendship exists');
        const now = Date.now();
        const times = (rate.data()?.times ?? []).filter((time) => now - time < 86_400_000 && time <= now);
        if (times.filter((time) => now - time < 3_600_000).length >= 10 || times.length >= 50)
            throw new HttpsError('resource-exhausted', 'Friend request limit');
...
export const respondFriendRequest = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!snapshot.exists || snapshot.data()?.recipientId !== recipientId || snapshot.data()?.status !== 'pending')
            throw new HttpsError('not-found', 'Pending request unavailable');
        if (accept) {
...
export const inviteToRoom = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (!pair.exists || pair.data()?.status !== 'accepted')
...
export const spendGift = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    assertSafeId(roomId);
    assertSafeId(idempotencyKey);
    if (senderId === recipientId)
        throw new HttpsError('invalid-argument', 'No self gift');
    ...
    const receipt = await db.runTransaction(async (transaction) => {
        ...
        if (existing.exists) {
            ...
            if (data === undefined)
                throw new HttpsError('internal', 'Stored receipt is unavailable');
            if (!sameGiftReceipt(data, { senderId, recipientId, giftId, roomId }))
                throw new HttpsError('already-exists', 'Idempotency conflict');
            return data;
        }
        if (block.exists)
            throw new HttpsError('permission-denied', 'Blocked users cannot exchange gifts');
        const balance = Number(wallet.data()?.balance ?? 0);
        try {
            assertGiftBalance(balance, cost);
        }
        catch {
            throw new HttpsError('failed-precondition', 'Insufficient chips');
        }
        let rateState;
        try {
            rateState = nextGiftRate(rate.data(), now, cost);
        }
        catch (error) {
            throw new HttpsError('resource-exhausted', error instanceof Error ? error.message : 'Gift rate limit');
        }
...
export async function removeStaleTokens(tokens, invalidTokens) {
    ...
    for (const doc of matching.docs)
        if (invalid.has(String(doc.data().token))) {
...
export async function sendPush(tokens, type, notificationId, deepLinkId) {
