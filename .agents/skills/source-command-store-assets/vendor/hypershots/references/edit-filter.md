...
>
> ```bash
> bash <skill>/scripts/edit-pass.sh <ws> <profile> <locale> <panel-N> "<style prompt>" [protected] [style-ref]
...
## App-factory approval receipt

Before `edit-pass.sh` can upload anything, `out/<profile>/review.html` must exist and the
clean set must have been shown to the user. After showing the exact panels, protected
regions, prompt, `openai/gpt-image-2/edit` model, and estimated total cost, record the
user's fresh approval in `<ws>/style-approval.json` with mode `0600`:
...
```json
{
  ...
  "cleanReviewSha256": "<sha256 of out/iphone-6.9/review.html>",
  "promptSha256": "<sha256 of the exact style prompt>",
  "estimatedCostUSD": 0.0,
  "approvedAt": "<ISO-8601 timestamp>"
}
```

The receipt contains no API key. Any changed review, prompt, panel, provider, model, or
cost invalidates it. `verify-provider-approval.mjs` runs before the first `genmedia` call.
...
- **full:** intentionally disabled in app-factory because it can redraw marketing copy,
  device geometry, and real app UI. Restyle `theme.css` and re-render instead.

If a panel's `boxes.json` has zero protect boxes, protected mode has nothing to protect — that's an authoring bug (see create.md marker rules), not a reason to bypass protection.

**Full-canvas protection (verified live):** the inverse failure. On a full-bleed layout — copy block spanning the top plus a large device — the padded protect union can cover the ENTIRE canvas. Protected mode then has nothing to edit: the composite would restore every pixel, and the model returns junk for an all-opaque mask (a blank paper texture in the live test), so the API call is a guaranteed paid no-op. `edit-pass.sh` detects this after building the mask and refuses **before** any upload. Restyle the theme and re-render instead.

**Palette-shift caveat (verified live):** the composite restores the ORIGINAL colors inside the protected halos, so a prompt that shifts the overall palette hard (e.g. a flat gray paper turning ochre) leaves the protection rectangles visible against the graded background, and the model may paint artifacts right along the mask edge. Grades that keep the panel's overall palette (grain, texture, subtle duotone within the theme's hues) blend best; for drastic re-colors, restyle the theme and re-render instead.
...
```bash
bash <skill>/scripts/grade-set.sh <ws> <profile> <locale> "<style prompt>" [protected]
...
**Protected set constraint:** protected grading requires layouts with real unprotected background — frameless artifact panels, photo-bleed backgrounds, or offset/partial devices. On the default centered layout (full-width copy block + large centered device) the padded protect union may cover the whole canvas and `edit-pass.sh` refuses before spending anything. There, a deterministic theme restyle is the only supported makeover path.
