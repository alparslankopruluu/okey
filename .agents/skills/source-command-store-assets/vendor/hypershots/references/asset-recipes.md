...
Generated recipes need the `genmedia` CLI (fal.ai). If it is missing, use **Degraded
mode** below—never block create/revise/translate on it. In app-factory, do not run raw
provider commands from this reference. The gated scripts first verify a 0600
`provider-approval.json` receipt against the already-shown clean review, exact artifact,
prompt, provider, model chain, and estimated cost.
...
```bash
bash <skill>/scripts/fal-sticker-pass.sh <ws> <profile> <locale> <name> "<SUBJECT>"
```
...
```bash
bash <skill>/scripts/fal-background-pass.sh <ws> <profile> <locale> <name> "<scene>"
```

The matching receipt uses artifact `sticker:<name>` with model
`openai/gpt-image-2+fal-ai/birefnet/v2`, or artifact `background:<name>` with model
`fal-ai/flux/schnell`. Use `promptSha256` for the exact generated prompt and
`cleanReviewSha256` for `out/<profile>/review.html`. The receipt never stores `FAL_KEY`.
