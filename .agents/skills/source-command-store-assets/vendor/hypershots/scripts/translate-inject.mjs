...
if (!/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(locale)) {
  console.error(`ERROR: invalid BCP-47 locale: ${locale}`); process.exit(1);
}
const parsed = JSON.parse(readFileSync(join(ws, `strings.${locale}.json`), 'utf8'));
if (parsed.locale && parsed.locale !== locale) {
  console.error(`ERROR: strings locale ${parsed.locale} does not match requested ${locale}`); process.exit(1);
}
...
for (const [key, value] of Object.entries(strings)) {
  if (typeof value !== 'string') {
    console.error(`ERROR: translation ${key} must be a string`); process.exit(1);
  }
  if (value.includes('\uFFFD')) {
    console.error(`ERROR: translation ${key} contains a replacement glyph`); process.exit(1);
  }
}
const outDir = join(ws, `panels-${locale}`);
const direction = ['ar', 'he', 'fa', 'ur'].includes(locale.toLowerCase().split('-')[0]) ? 'rtl' : 'ltr';
...
for (const f of readdirSync(join(ws, 'panels')).filter(f => f.endsWith('.html'))) {
  ...
  if (!/<html\b/i.test(html)) {
    console.error(`ERROR: ${f} has no html root`); process.exit(1);
  }
  html = html.replace(/<html\b([^>]*)>/i, (_match, attributes) => {
    const clean = attributes
      .replace(/\s(?:lang|dir)=(?:"[^"]*"|'[^']*')/gi, '')
      .trim();
    return `<html${clean ? ` ${clean}` : ''} lang="${locale}" dir="${direction}">`;
  });
  html = html.replace(
