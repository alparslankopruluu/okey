...
# Usage:
#   grade-set.sh <workspace> <profile> <locale> "<style prompt>" [protected]
set -euo pipefail
WS="${1:?usage: grade-set.sh <workspace> <profile> <locale> \"<style prompt>\" [protected]}"
PROFILE="${2:?usage: grade-set.sh <workspace> <profile> <locale> \"<style prompt>\" [protected]}"
LOCALE="${3:?usage: grade-set.sh <workspace> <profile> <locale> \"<style prompt>\" [protected]}"
STYLE="${4:?usage: grade-set.sh <workspace> <profile> <locale> \"<style prompt>\" [protected]}"
MODE="${5:-protected}"
...
case "$MODE" in
  protected) ;;
  full) echo "ERROR: app-factory forbids full-canvas AI edits because they can redraw protected store content" >&2; exit 1 ;;
