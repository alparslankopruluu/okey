...
| Deterministic Classic/101 game model, seeded shuffle/bot, replay and core rule validation | offline, game | local only | TR + EN | game core |
| Offline four-seat turn loop with validated/versioned resume snapshot, rack reordering, horizontal two-digit tile glyphs and a responsive two-shelf rack | offline, game | local only | TR + EN | offline mode |
